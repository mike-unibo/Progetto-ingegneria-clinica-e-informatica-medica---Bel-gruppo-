...
st.set_page_config(page_title="WearGait-PD · Balance - group analysis", layout="wide")

COLOR_MAP = {"PD": "crimson", "Control": "royalblue"}

# Palette combinata per Grafico a Torta e Scatter Plot
COMBINED_COLORS = {
    "PD — M": "crimson",
    "PD — F": "salmon",
    "Control — M": "royalblue",
...
st.write("I partecipanti PD e Control hanno eseguito 6 diversi sottocompiti di equilibrio: occhi aperti/chiusi e diverse posizioni dei piedi")
st.write("In questa sezione puoi esplorare i dati di equilibrio filtrando per gruppo, sesso ed età, e visualizzare le distribuzioni delle metriche di equilibrio sia per le solette che per la pedana. Puoi anche eseguire test statistici per confrontare i gruppi PD e Control e visualizzare scatter plot per analizzare le relazioni tra le metriche di equilibrio e le variabili demografiche.")
...
def show_scatter_metrcs(filtered_df, demo_var, scatter_metric):
    ...
    st.dataframe(pd.DataFrame(metrics_list),width="stretch", hide_index=True,width='stretch',
    column_config={
        "Spearman ρ": st.column_config.NumberColumn(
            "Spearman ρ",
            help=(
                "Misura la forza e la direzione della relazione tra la variabile "
                "demografica e la metrica del cammino.\n\n"
        ...
        "p-value": st.column_config.NumberColumn(
            "p-value",
            help=(
                "Indica l'affidabilità statistica della correlazione di Spearman. "
            ),
            format="%.4f"
        ),
        "Pendenza OLS": st.column_config.NumberColumn(
            "Pendenza OLS",
            help=(
                "Inclinazione della retta di regressione calcolata con il metodo "
...
with st.expander("SCATTER PLOT"):
    ...
    if len(filtered_df[D_globale[y_metriche]]) == 0 or len(filtered_df[D_demografiche[x_demo]]) == 0:
        ...
        show_scatter_metrcs(filtered_df, D_demografiche[x_demo], D_globale[y_metriche])