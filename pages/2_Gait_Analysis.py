...
# ── VERIFICA SE SONO ATTIVI FILTRI ───────────────────────────────────────────
filtri_attivi = (
    use_balanced
    or group_filter != "Entrambi"
    or sex_filter != "Entrambi"
    or age_range != (age_min, age_max)
)
...
def apply_global_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if sex_filter == "Maschio":
        df = df[df["Sex"] == "Male"]
    elif sex_filter == "Femmina":
        df = df[df["Sex"] == "Female"]
    return df
...
# ── CONTATORI: originali se no filtri, aggiornati se filtri attivi ────────────
if filtri_attivi:
    filt_total = len(df_plot)
...
# ── "SONO STATI FILTRATI X CAMPIONI" ─────────────────────────────────────────
if filtri_attivi:
    TOTALE_FISSO = 134
    campioni_esclusi = TOTALE_FISSO - filt_total
else:
...
with comp_col:
    ...
    # Invalidi: mostrati solo se NON ci sono filtri attivi (comportamento originale)
...
with age_col:
    # Istogramma età: df_filtered se no filtri (originale), df_plot se filtri attivi
    if filtri_attivi:
        df_age = df_plot[df_plot["PD_vs_Control"].isin(["PD", "Control"])].dropna(subset=["Age"])
    else:
