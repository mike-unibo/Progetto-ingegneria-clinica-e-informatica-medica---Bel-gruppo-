...
def show_pressure_metrics(data):
    ...
    tot_l = np.sum(lp, axis=1)
    tot_r = np.sum(rp, axis=1)
    mean_l = float(np.mean(tot_l))
    mean_r = float(np.mean(tot_r))
    max_p = float(max(np.max(lp), np.max(rp)))
    asymmetry = ((mean_r - mean_l) / (mean_r + mean_l)) * 100 if (mean_l + mean_r) > 0 else 0
    
    with st.container(border=True):
        col1, col2, col3= st.columns(3)
        col1.metric("Carico medio SX (N/cm²)", f"{mean_l:.2f}", border=True, help="Forza totale media esercitata dal piede sinistro durante i 10 secondi.\n\nÈ rilevante perché permette di capire quanto peso il paziente scarica su ciascun piede — un carico medio molto diverso tra piede sinistro e destro indica uno sbilanciamento posturale, frequente nel Parkinson dove la rigidità e la bradicinesia tendono a essere asimmetriche.")
        col2.metric("Carico medio DX (N/cm²)", f"{mean_r:.2f}", border=True, help="Forza totale media esercitata dal piede destro durante i 10 secondi.\n\nÈ rilevante perché permette di capire quanto peso il paziente scarica su ciascun piede — un carico medio molto diverso tra piede sinistro e destro indica uno sbilanciamento posturale, frequente nel Parkinson dove la rigidità e la bradicinesia tendono a essere asimmetriche.")
        col3.metric("Pressione max (N/cm²)", f"{max_p:.2f}", border=True)
        col1, col2, col3= st.columns(3)
...
def show_pressure_graph(data, mask_L, mask_R, WL, WR):
    ...
    st.caption("La scala colori delle sagome è normalizzata al 98° percentile delle pressioni, per evitare che picchi isolati riducano la leggibilità della mappa.  \nLa pressione massima riportata nelle metriche indica invece il picco reale registrato.", text_alignment="center")
    st.write("")
...
def render_cop_metrics(m, pid=None):
    ...
    with st.container(border=True):
        ...
        col2.metric("Area ellisse 95% (cm²)", f"{m['area_95']:.2f}", border=True, help="Area dell'ellisse che racchiude il 95% dei punti CoP registrati durante i 10 secondi.  \nUn'area più grande indica una maggiore dispersione del CoP e quindi minore stabilità posturale.")


def show_cop_metrics(data1, pid1, data2=None, pid2=None):
    """ Calcola e visualizza le metriche posturali del centro di pressione. """
    m1 = cop_metrics(data1['cop_bx'], data1['cop_by'], data1['ts'])
    if data2 is None:
        render_cop_metrics(m1)
    else:
        m2 = cop_metrics(data2['cop_bx'], data2['cop_by'], data2['ts'])
...
def show_cop_section(data1, pid1, data2=None, pid2=None):
    ...
    if data2 is None:
        ...
        with center:
            st.plotly_chart(make_fig(data1, pid1), width='stretch')
        ...
        with col1:
            st.plotly_chart(make_fig(data1, pid1), width='stretch')
        with col2:
            st.plotly_chart(make_fig(data2, pid2), width='stretch')
